...
class Admin extends CI_Controller
{
	...
	function student_information($class_id = '')
	{
		...
		$page_data['page_title'] 	= get_phrase('List_of_students')." : ".
    ...
    function student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
			...
			$con = mysqli_connect("localhost","root","","champs");
			...
			if (mysqli_connect_errno())
			  ...
			  $result = mysqli_query($con,"SELECT * FROM student ORDER BY roll DESC LIMIT 1");
			   while($row = mysqli_fetch_array($result)) {
					$lastidd= $row['student_id'];
				}
			  $lastidd=++$lastidd;
			$data['roll']        = 'CLHO'.date("m").date("Y").$lastidd;
    ...
    function teacher($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {            
            ...
            $data['email']       = $this->input->post('email');
            $data['password']    = $this->input->post('password');
			$data['qtsqyaulify']       = $this->input->post('qtsqyaulify');
            $data['qtsexpiry']    = $this->input->post('qtsexpiry');
			$data['onlineservice']       = $this->input->post('onlineservice');
            $data['dbsissudate']    = $this->input->post('dbsissudate');
			$data['srname']       = $this->input->post('srname');
			$data['wphone']    = $this->input->post('wphone');
			$data['hphone']       = $this->input->post('hphone');
            $data['pcode']    = $this->input->post('pcode');
			$data['certifinumber']    = $this->input->post('certifinumber');
            $this->db->insert('teacher', $data);
            $teacher_id = mysql_insert_id();
			move_uploaded_file($_FILES['expcerti']['tmp_name'], 'uploads/teacher_image/' . $teacher_id . '.jpg');
			move_uploaded_file($_FILES['educerti']['tmp_name'], 'uploads/teacher_image/' . $teacher_id . '.jpg');
        ...
        if ($param1 == 'do_update') {
			...
			move_uploaded_file($_FILES['expcerti']['tmp_name'], 'uploads/teacher_image/' . $param2 . '.jpg');
			move_uploaded_file($_FILES['educerti']['tmp_name'], 'uploads/teacher_image/' . $param2 . '.jpg');
            redirect(base_url() . 'index.php?admin/teacher/', 'refresh');
	 ...
	 /****MANAGE TEACHERS online_applications*****/
    function online_applications($param1 = '', $param2 = '', $param3 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
       
       
        $page_data['teachers']   = get_phrase('Career Section is yet to be intigrate');
        $page_data['page_name']  = 'online_applications';
        $page_data['page_title'] = get_phrase('Career Section is yet to be intigrate with admin panel');
        $this->load->view('backend/index', $page_data);
    }
	
	 /****MANAGE TEACHERS new_registrations*****/
    function new_registrations($param1 = '', $param2 = '', $param3 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        ...
        $page_data['teachers']   = get_phrase('Career Section is yet to be intigrate');
        $page_data['page_name']  = 'new_registrations';
        $page_data['page_title'] = get_phrase('Register new Teachers');
        $this->load->view('backend/addteacher', $page_data);
    }
    ...
    function subject($param1 = '', $param2 = '' , $param3 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        if ($param1 == 'create') {
            $data['yearn']       = $this->input->post('yearn');
            $data['subject']   = $this->input->post('subject');
            $data['sdate'] = $this->input->post('sdate');
			$data['edate'] = $this->input->post('sdate');
            $this->db->insert('subject', $data);
            redirect(base_url() . 'index.php?admin/subject/1', 'refresh');
        }
        if ($param1 == 'do_update') {
            $data['yearn']       = $this->input->post('yearn');
            $data['subject']   = $this->input->post('subject');
    ...
    function classes($param1 = '', $param2 = '')
    {
        ...
        if ($param1 == 'create') {
            $data['name']         = $this->input->post('name');
            $data['subject'] = $this->input->post('subject');
            $data['yeaar']   = $this->input->post('yeaar');
			$data['teacher']         = $this->input->post('teacher');
            $data['start_date'] = $this->input->post('start_date');
            $this->db->insert('class', $data);
            redirect(base_url() . 'index.php?admin/classes/1', 'refresh');
        }
        if ($param1 == 'do_update') {
            $data['name']         = $this->input->post('name');
            $data['subject'] = $this->input->post('subject');
		...
		$page_data['class_id']   = $param1;
		 ...
		 if ($param1 == '2') {
				//$page_data['subjects']   = $this->db->get_where('subject' , array('class_id' => $param1))->result_array();
        		$page_data['page_title'] = get_phrase('new_class_allocation');
		}
	    ...
	    /****MANAGE TUTIONS*****/
    function tution_centers($param1 = '', $param2 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        if ($param1 == 'create') {
            $data['center_no']         = $this->input->post('center_no');
            $data['center_name'] = $this->input->post('center_name');
            $data['center_manger']   = $this->input->post('center_manger');
			$data['center_address']         = $this->input->post('center_address');
            $data['center_contact'] = $this->input->post('center_contact');
            $this->db->insert('tuition', $data);
            redirect(base_url() . 'index.php?admin/tution_centers/1', 'refresh');
        }
        if ($param1 == 'do_update') {
            $data['center_no']         = $this->input->post('center_no');
            $data['center_name'] = $this->input->post('center_name');
            ...
            $this->db->where('tution_id', $param2);
            $this->db->update('tuition', $data);
            redirect(base_url() . 'index.php?admin/tution_centers/1', 'refresh');
        } else if ($param1 == 'edit') {
            $page_data['edit_data'] = $this->db->get_where('tuition', array(
                'tution_id' => $param2
            ))->result_array();
        }
        if ($param1 == 'delete') {
            $this->db->where('tution_id', $param2);
            $this->db->delete('tuition');
            redirect(base_url() . 'index.php?admin/tution_centers/1', 'refresh');
        }
        //$page_data['classes']    = $this->db->get('class')->result_array();
		$page_data['class_id']   = $param1;
		 if ($param1 == '1') {
			$page_data['tuition']   = $this->db->get('tuition')->result_array();
        	$page_data['page_title'] = get_phrase('Manage_Tuition_Centres');
		}
		 if ($param1 == '2') {
				//$page_data['subjects']   = $this->db->get_where('subject' , array('class_id' => $param1))->result_array();
        		$page_data['page_title'] = get_phrase('Add_new_Tuition_Centres');
		}
        $page_data['page_name']  = 'tution_centers';
        $this->load->view('backend/index', $page_data);
    }
	
	
	    /****VIEW TUTIONS*****/
    function view_tution($param1 = '', $param2 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        if ($param1 == 'create') {
		...
		$page_data['tuition']   = $this->db->get_where('tuition' , array('tution_id' => $param2) )->result_array();
		$page_data['rooms']   = $this->db->get('rooms')->result_array();
        $page_data['page_name']  = 'view_tution';
        $this->load->view('backend/index', $page_data);
    }
    ...
    function manage_profile($param1 = '', $param2 = '', $param3 = '')
    {
	 ...
	 /****MANAGE USERS*****/
    function users($param1 = '', $param2 = '', $param3 = '', $param4 = '')
    {
        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');
        if ($param1 == 'create') {
            ...
            redirect(base_url() . 'index.php?admin/users/2', 'refresh');
        }
        if ($param1 == 'do_update') {
             $data['name']         = $this->input->post('name');
            $data['email'] = $this->input->post('email');
        ...
        if ($param1 == 'delete') {
		 ...
		 if ($param1 == 'status') {
            ...
            if($param3=='teacher'){
				...
				}elseif($param3=='student'){
					
					$this->db->where('student_id', $param2);
                    $this->db->update('student', $data);
					
					}elseif($param3=='parent'){
		...
		$page_data['admin_id']   = $param1;
		 if ($param1 == '2') {
			...
			$page_data['teacherusers']   = $this->db->get('teacher')->result_array();
			$page_data['studentusers']   = $this->db->get('student')->result_array();
			$page_data['parentusers']   = $this->db->get('parent')->result_array();
			
        	$page_data['page_title'] = get_phrase('manage_users');
		}
		 if ($param1 == '1') {
				//$page_data['subjects']   = $this->db->get_where('subject' , array('class_id' => $param1))->result_array();
        		$page_data['page_title'] = get_phrase('add_admins');
		}
